    ...
    IKiteStakingManager
{
    ...
    struct KiteStakingManagerStorage {
        ...
        /// @notice Whether new validator and delegator registrations are paused
        bool registrationPaused;
        /// @notice Addresses with the operator role
        mapping(address => bool) operators;
    }

    // Hardcoded storage slot for KiteStakingManagerStorage.
    // NOTE: This value does not match the ERC-7201 formula for
    // "avalanche-icm.storage.KiteStakingManager" but is retained
    // for deployed storage compatibility.
    ...
    modifier whenRegistrationNotPaused() {
        if (_getKiteStakingManagerStorage().registrationPaused) {
            revert RegistrationPaused();
        }
        _;
    }
    ...
    function _reward(
        ...
        try vault.distributeReward(account, amount) {
            return true;
        } catch {
            emit RewardDistributionFailed(
                account,
    ...
    /**
     * @notice See {IKiteStakingManager-setRegistrationPaused}.
     */
    function setRegistrationPaused(bool paused) external onlyOperator {
        _getKiteStakingManagerStorage().registrationPaused = paused;
        emit RegistrationPauseStatusChanged(paused);
    }

    /**
     * @notice See {IKiteStakingManager-isRegistrationPaused}.
     */
    function isRegistrationPaused() external view returns (bool) {
        return _getKiteStakingManagerStorage().registrationPaused;
    }

    /**
     * @notice See {IKiteStakingManager-grantOperatorRole}.
     */
    function grantOperatorRole(address account) external onlyOwner {
        _getKiteStakingManagerStorage().operators[account] = true;
        emit OperatorRoleUpdated(account, true);
    }

    /**
     * @notice See {IKiteStakingManager-revokeOperatorRole}.
     */
    function revokeOperatorRole(address account) external onlyOwner {
        _getKiteStakingManagerStorage().operators[account] = false;
        emit OperatorRoleUpdated(account, false);
    }

    /**
     * @notice See {IKiteStakingManager-isOperator}.
     */
    function isOperator(address account) external view returns (bool) {
        return _getKiteStakingManagerStorage().operators[account];
    }

    // ============================================
    // Admin Force Remove Validator
    // ============================================

    /**
     * @notice See {IKiteStakingManager-forceRemoveValidator}.
     */
    function forceRemoveValidator(
        ...
        _adminForceRemoveValidator(
            validationID,
            includeUptimeProof,
            messageIndex,
            forfeitRewards
        );
        emit AdminForceValidatorRemoval(validationID, forfeitRewards);
    }
