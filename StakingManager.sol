    ...
    ReentrancyGuardUpgradeable
{
    ...
    struct StakingManagerStorage {
        ...
        /// @notice Maps the delegation ID to the earliest timestamp at which its churn lock expires.
        mapping(bytes32 delegationID => uint64) _delegationChurnLockUntil;
    }
    ...
    /**
     * @notice See {IStakingManager-claimValidatorRewardsFor}.
     * Allows anyone to trigger a reward claim for an active validator on behalf of its owner.
     * Rewards are sent to the validator's registered reward recipient.
     */
    function claimValidatorRewardsFor(
        bytes32 validationID,
        bool includeUptimeProof,
        uint32 messageIndex
    ) external nonReentrant returns (uint256 reward) {
    ...
    function _claimActiveValidatorRewards(
        ...
        emit ValidatorRewardClaimed(validationID, rewardRecipient, reward);
    }
    ...
    /**
     * @notice See {IStakingManager-claimDelegatorRewardsFor}.
     * Allows anyone to trigger a reward claim for an active delegator on behalf of its owner.
     * Rewards are sent to the delegator's registered reward recipient.
     */
    ...
    /**
     * @dev Admin-only helper that initiates validator removal without owner or minStakeDuration checks.
     * If forfeitRewards is true, no rewards are calculated; otherwise rewards are computed normally.
     */
    function _adminForceRemoveValidator(
        bytes32 validationID,
        bool includeUptimeProof,
        uint32 messageIndex,
        bool forfeitRewards
        ...
        // Always update uptime regardless of forfeitRewards, so that
        // delegator reward calculations use an accurate uptime value.
        // After this call the validator is PendingRemoved and no further
        // uptime updates are possible.
        uint64 uptimeSeconds;
        if (includeUptimeProof) {
            uptimeSeconds = _updateUptime(validationID, messageIndex);
        } else {
        ...
        if (forfeitRewards) {
            // Zero out the fee rate so all existing commission-calculation paths
            // naturally produce validatorFee = 0, letting delegators keep the
            // full gross reward.
            $._posValidatorInfo[validationID].delegationFeeBips = 0;
            delete $._redeemableValidatorRewards[validationID];
        } else {
            PoSValidatorInfo storage posInfo = $._posValidatorInfo[
                validationID
            ];
            uint64 lastClaimTime = posInfo.lastRewardClaimTime;
            uint64 lastClaimUptime = posInfo.lastClaimUptimeSeconds;
    ...
    function _initiateValidatorRegistration(
        ...
        _validateStakeAmountMultiple(stakeAmount);
    ...
    function valueToWeight(uint256 value) public view returns (uint64) {
        StakingManagerStorage storage $ = _getStakingManagerStorage();
        if (value % $._weightToValueFactor != 0) {
            revert InvalidStakeAmount(value);
        }

        uint256 weight = value / $._weightToValueFactor;
    ...
    /**
     * @dev Ensures the provided stake amount can be represented exactly as validator weight.
     */
    function _validateStakeAmountMultiple(uint256 value) internal view {
        if (value % _getStakingManagerStorage()._weightToValueFactor != 0) {
            revert InvalidStakeAmount(value);
        }
    }
    ...
    function _initiateDelegatorRegistration(
        ...
        _validateStakeAmountMultiple(delegationAmount);
        ...
        $._delegatorRewardRecipients[delegationID] = rewardRecipient;
        $._delegationChurnLockUntil[delegationID] =
            uint64(block.timestamp) +
            $._manager.getChurnPeriodSeconds();
    ...
    function completeDelegatorRegistration(
        ...
        // Once the validator has started exiting, this delegation can no longer become active.
        // Fall through directly to the removal/refund path instead of activating it.
        if (
            validator.status == ValidatorStatus.Completed ||
            validator.status == ValidatorStatus.PendingRemoved
        ) {
        ...
        // Initialize lastClaimUptimeSeconds to current validator uptime.
        // This ensures new delegators only earn rewards from their registration time,
        // not from the validator's entire uptime history.
        //
        // Note: The stored uptimeSeconds may not be the absolute latest value, but we mitigate
        // this through: (1) periodic uptime proof submissions (e.g., per 15 mins) for all validators,
        // and (2) uptime updates triggered by reward claims. This keeps the deviation minimal
        // and acceptable for reward calculations.
        // Ideally, uptime proofs should be bound to a verifiable generation time
        // (e.g., include a timestamp or slot in the signed message), which requires node-level support.
    ...
    function _initiateDelegatorRemoval(
        ...
        if (validator.status == ValidatorStatus.Active) {
            // Check that minimum stake duration has passed.
            // Uses the validator's per-registration minStakeDuration rather than the
            // mutable global _minimumStakeDuration, so that admin config changes do
            // not retroactively extend existing delegators' lock periods.
            if (
                block.timestamp <
                delegator.startTime +
                    $._posValidatorInfo[validationID].minStakeDuration
    ...
    function resendUpdateDelegator(bytes32 delegationID) external {
        ...
        if (validator.sentNonce == validator.receivedNonce) {
            return;
        }
    ...
    function _completeDelegatorRemoval(bytes32 delegationID) internal {
        ...
        Delegator memory delegator = $._delegatorStakes[delegationID];
        bytes32 validationID = delegator.validationID;
        uint64 churnLockUntil = $._delegationChurnLockUntil[delegationID];

        // Fallback to the historical startTime-based behavior for delegations
        // created before the churn-lock field existed.
        if (churnLockUntil == 0) {
            churnLockUntil =
                delegator.startTime +
                $._manager.getChurnPeriodSeconds();
        ...
        if (block.timestamp < churnLockUntil) {
            revert MinStakeDurationNotPassed(uint64(block.timestamp));
        }
        ...
        delete $._delegationChurnLockUntil[delegationID];
     ...
     *    - For Delegators: Only affects NEW delegations indirectly (delegator exit uses the parent validator's
     *      per-registration minStakeDuration, not this global value). Changing this value does NOT retroactively
     *      affect existing delegators' lock periods.
     */
